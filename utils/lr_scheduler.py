...
def get_scheduler(optimizer, args):
    if "cosine" in args.lr_scheduler:
        scheduler = CosineAnnealingLR(
            optimizer=optimizer,
            eta_min=0.000001,
            T_max=(args.nepoch - args.warmup_epoch))
    elif "step" in args.lr_scheduler:
        lr_decay_epochs = [args.lr_decay_steps * i for i in range(1, args.nepoch // args.lr_decay_steps)]
        scheduler = MultiStepLR(
            optimizer=optimizer,
            gamma=args.lr_decay_rate,
            milestones=[(m - args.warmup_epoch) for m in lr_decay_epochs])
    else:
