...
def cfg_from_list(cfg_list, config):
    """Set options keys via list (e.g., from command line)."""
...
def backup_files(opt):
    path = opt.log_dir
    os.makedirs(os.path.join(path, 'backup'), exist_ok=True)
    shutil.copy('models/' + opt.model + '.py', str(os.path.join(path, 'backup')))
    shutil.copy('datasets/' + opt.dataset + '/protein_dataset.py', str(os.path.join(path, 'backup')))
    shutil.copy('train.py', str(os.path.join(path, 'backup')))
    shutil.copy(opt.cfg_file, str(path))
